...
void AProjectile::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComponent, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    ATPSworkGameMode* TPSGameM = Cast<ATPSworkGameMode>(GetWorld()->GetAuthGameMode());
    if (TPSGameM == nullptr) {
        GEngine->AddOnScreenDebugMessage(0, 10.0f, FColor::Yellow, "gamemode is null");
        return;
    }
    ...
    // -1"键"值（首个参数）说明我们无需更新或刷新此消息。
        ...
        TPSGameM->Score += AddScore;
        GEngine->AddOnScreenDebugMessage(0, 99999.0f, FColor::Yellow, "Get Score " + FString::FromInt(AddScore) + "! Your total score is " + FString::FromInt(TPSGameM->Score) + ".");
        return;
